...
import { ObtenerRecetaById, crearRecetas, modificarRecetas } from '../../api/requestmaker';
...
const EditarReceta = () => {
    ...
    useEffect(()=>{
        ObtenerRecetaById(id).then((respuesta)=>{
          if(respuesta){
            ...
            setUrl_img(respuesta.url_img)
          }else {
            setMensaje('No se pudo cargar la receta')
            setTipo('error');
            setOpen(true)
          }
        })
    ...
    const onAddClick = async ()=>{
        ...
        await modificarRecetas(id,recetasCreada)
        setMensaje('Se pudo modificar la receta')
        setTipo('success')
        setOpen(true)
        setTimeout(() => {
            navigate('/')
